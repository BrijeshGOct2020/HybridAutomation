...
public class PredefinedActions {
	...
	protected WebElement getElement(String locator, boolean isWaitRequired) {
		...
		switch(locatorType.toUpperCase()) {
			...
			case "PARTIALLINK":
				...
				break;
			default:
				throw new InvalidLocatorStrategy("Please use valid locator strategy from below\nclass, id, link, partialLink, name, tag, xpath, css");
		}
		return element;
	}
	...
	protected List<WebElement> getElements(String locator, boolean isWaitRequired) {
		...
		switch(locatorType.toUpperCase()) {
			...
			case "PARTIALLINK":
				if(isWaitRequired)
					elements = wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.partialLinkText(locatorValue)));
				else
	...
	public static void captureScreenshot(String name) {
		...
		try {
			Files.copy(file, new File("./Screenshots/"+name+"_"+TimeUtil.getTimeStamp()+".png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
